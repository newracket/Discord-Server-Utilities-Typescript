...
export default class ModifyRoleCommand extends Command {
  ...
  async execute(message: Message | CommandInteraction, args: { type: string, role: Role, value: string }) {
    switch (args.type.toLowerCase()) {
      case "name":
        await args.role.setName(args.value);
        return await message.reply(`The new name of your role is ${args.value}`);
      case "color":
        const newColor = Utils.getHexFromString(args.value);
        if (newColor === null) return await message.reply("The color is invalid");

        await args.role.setColor(newColor);
        return await message.reply(`The new color of ${args.role.name} is ${args.role.hexColor}`);
      default:
