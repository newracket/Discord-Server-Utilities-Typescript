...
export default class ChangeNickCommand extends Command {
  ...
  async execute(message: Message | CommandInteraction, args: { member: GuildMember, nick: string }) {
    await args.member.setNickname(args.nick === "default" ? null : args.nick);
    await message.reply(`Nickname changed for ${args.member}. New nickname is ${args.nick}.`);
  }
}