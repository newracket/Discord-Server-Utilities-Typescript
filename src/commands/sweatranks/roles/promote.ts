...
export default class PromoteCommand extends Command {
  ...
  constructor() {
    super({
      ...
      usage:
        "promote <mention members> <amount> OR promote <member nicknames> <amount>",
      category: "Sweatranks",
      channel: "guild",
  ...
  async execute(
    ...
    if (args.member.length == 0 && message instanceof Message) {
      ...
      if (args.member.length == 0)
        return message.reply("You didn't specify anyone to promote.");
    }
    ...
    if (casranks.filter((rank) => roles.includes(rank)).length > 0) {
      ...
      if (strikes[member.id] == undefined) {
        ...
        if (strikes[member.id].value == 3) {
          ...
          this.messagesToSend[member.displayName].push(
            `${member} was given his last strike. He has now been promoted.`
          );
          ...
          this.messagesToSend[member.displayName].push(
            `${member} was given his second strike.`
          );
          strikesJSON.set(strikes);
      ...
      if (lastRank === undefined)
        return message.reply("Error in code: lastrank is undefined");
