import { Collection, CommandInteraction, GuildMember, Message, RoleResolvable } from "discord.js";
...
export default class GiveCasCommand extends Command {
  ...
  constructor() {
    super({
      ...
      usage: "givecas <mention members> <amount> OR givecas <member nicknames> <amount>",
      category: "Sweatranks",
      channel: "guild",
  ...
  async givecas(message: Message | CommandInteraction, member: GuildMember): Promise<any> {
    if (message.guild === null) return message.reply("Guild does not exist");

    const lastRank = casranks.filter(rank => member.roles.cache.map(role => role.name).includes(rank)).pop() as string;

    if (casranks.indexOf(lastRank) === casranks.length - 1) {
      return message.reply(`This person is already ${lastRank} and cannot get any more cas roles.`);
    }
    else if (lastRank !== undefined) {
      const newCasRole = message.guild.roles.cache.find(role => role.name == casranks[casranks.indexOf(lastRank) + 1]);
      await member.roles.add(newCasRole as RoleResolvable)
      message.reply(`Successfully gave ${casranks[casranks.indexOf(lastRank) + 1]} to <@${member.id}>`);
    }
    else {
