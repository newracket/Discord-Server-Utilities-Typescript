...
export default class DemoteCommand extends Command {
  ...
  constructor() {
    super({
      ...
      usage: "demote <mention members> <amount> OR demote <member nicknames> <amount>",
      category: "Sweatranks",
      channel: "guild",
  ...
  async demoteMember(message: Message | CommandInteraction, member: GuildMember, roles: any, repeatTimes: number): Promise<any> {
    ...
    if (lastRank !== undefined && lastRank !== "Member") {
      ...
      if (lastRankIndex === casranks.length - 1 && !strikesJSON.hasKey(member.id)) {
      ...
      if (strikesJSON.hasKey(member.id)) {
        const strikesMessage = await Utils.resolveMessage(strikesChannelId, strikesJSON.getValue(member.id).messageId, message);
        ...
        this.messagesToSend[member.displayName].push(`${member} was demoted to ${casranks[lastRankIndex + 1]} with 2 strikes.`);
        roles.push(casranks[lastRankIndex + 1]);

        const strikesChannel = await Utils.resolveChannel(strikesChannelId, message);
        if (!(strikesChannel instanceof TextChannel)) return message.reply("Error in code: strikesChannel is undefined");
