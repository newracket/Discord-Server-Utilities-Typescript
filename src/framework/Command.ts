import { ApplicationCommandOptionData, Collection, CommandInteraction, CommandInteractionOption, GuildChannel, GuildMember, Message, Role, Snowflake, ThreadChannel } from "discord.js";
...
export default class Command {
  ...
  async checkPerms(client: CustomClient, message: Message | CommandInteraction, ignorePermissions: string[], givenArgs?: ArgumentReturnValue) {
    let authorId;
    ...
    if (this.ownerOnly && authorId !== client.ownerID) return await message.reply("Only the owner may execute this command.");
    if (this.channel === "guild" && message.guild === null) return await message.reply("You may only use this command in servers");
    if (this.channel === "dm" && message.guild !== null) return await message.reply("You may only use this command in dms");
    ...
    if (this.allowedRoles !== undefined
      ...
      return await message.reply(`Missing permissions. One of these roles are required: ${this.allowedRoles.map(e => message.guild?.roles.cache.get(e)?.name).join(", ")}`);
    }
  ...
  async findChannel(channels: Collection<Snowflake, GuildChannel>, words: string[]): Promise<GuildChannel | ThreadChannel | undefined> {
    let currentChannel = "";
